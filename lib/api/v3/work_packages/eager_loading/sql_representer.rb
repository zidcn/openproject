...
module API
  module V3
    module WorkPackages
      module EagerLoading
        class SqlRepresenter < Base
          def apply(work_package)
            work_package.json_representer = json_representer_for(work_package.id)
          end
          ...
          def json_representer_map
            sql = <<-SQL
               WITH visible_projects AS (#{::Project.allowed_to(User.current, :view_work_packages).select(:id).to_sql})
               ...
               (SELECT id, json_agg(child_hash) as children
                            FROM
                            (
                            SELECT relations.from_id AS id, json_build_object('href', '/api/v3/work_packages/' || children.id,
                            ...
                            WHERE children.project_id IN (SELECT id from visible_projects)
              ...
              (SELECT id, json_agg(ancestor_hash) as ancestors
                      ...
                      JOIN work_packages ancestors ON
                        ...
                        AND relations.duplicates = 0
                        AND relations.requires = 0
                      WHERE ancestors.project_id IN (SELECT id FROM visible_projects)
